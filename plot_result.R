...
compute_curve <- function(df, thresholds = seq(0, 100, by = 1)) {
  ...
  for (i in seq_along(thresholds)) {
    ...
    total_loss <- sum(hybrid_losss, na.rm = TRUE)
...
data=simulate_data_cls(n=6000, train_n=2000, test_n=3000,p=100,groups=6)
...
data=simulate_data_reg(n=18000, train_n=8000, test_n=9000,p=100,groups=6)
ELIE_result_reg=MRF_imputation_step_reg(y_name,block_1_names,imp_names,train_df,test_df,
                                        full_tree_size=10,red_tree_size=30,total_trees=1000)


# use regression example to show the results in the paper
#Histogram of ELIE
ggplot(data=ELIE_result_reg,aes(x=ELIE_avs))+
...
plot_result=compute_curve(df)
plot_result$percent=seq(0,100,1)
# SSE vs cumulative ELIE for observations with Z measured
ggplot(plot_result, aes(x =cumsum_ELIE , y = total_loss,color = Method)) +
  geom_line(size = 1.2)+
...
ggplot(plot_result, aes(x =percent , y = total_loss/nrow(data$test_df),color = Method)) +
  geom_line(size = 1.2)+
